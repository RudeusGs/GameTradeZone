...
export default {
  ...
  add: async (gameField: AddGameFieldModel): Promise<AxiosResponse<ApiResult<any>>> => {
    return await baseApi.post('GameField/Add', gameField, { params: { id: 0 } });
  },

  update: async (gameField: UpdateGameFieldModel): Promise<AxiosResponse<ApiResult<any>>> => {
    return await baseApi.post(`GameField/Update?id=${gameField.id}`, gameField, { params: { id: gameField.id } });
  },

  delete: async (id: number): Promise<AxiosResponse<ApiResult<any>>> => {
    return await baseApi.post(`GameField/Delete?id=${id}`, null, { params: { id } });
  },
